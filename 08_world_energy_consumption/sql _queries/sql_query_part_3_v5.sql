...
SELECT *, 
	hydro_electricity/SUM(hydro_electricity) OVER (PARTITION BY year ORDER BY year) AS hydro_elec_world_percent,
	other_renewable_electricity/SUM(other_renewable_electricity) OVER (PARTITION BY year ORDER BY year) AS other_renewable_elec_world_percent,
	solar_electricity/SUM(solar_electricity) OVER (PARTITION BY year ORDER BY year) AS solar_elec_world_percent,
	wind_electricity/SUM(wind_electricity) OVER (PARTITION BY year ORDER BY year) AS wind_elec_world_percent,
	renewable_elec_generated/SUM(renewable_elec_generated) OVER (PARTITION BY year ORDER BY year) AS renewable_elec_world_percent,
	
	coal_electricity/SUM(coal_electricity) OVER (PARTITION BY year ORDER BY year) AS coal_elec_world_percent,
	gas_electricity/SUM(gas_electricity) OVER (PARTITION BY year ORDER BY year) AS gas_elec_world_percent,
	nuclear_electricity/SUM(nuclear_electricity) OVER (PARTITION BY year ORDER BY year) AS nuclear_elec_world_percent,
	oil_electricity/SUM(oil_electricity) OVER (PARTITION BY year ORDER BY year) AS oil_elec_world_percent,
	non_renewable_elec_generated/SUM(non_renewable_elec_generated) OVER (PARTITION BY year ORDER BY year) AS non_renewable_elec_world_percent,

	electricity_generation/SUM(electricity_generation) OVER (PARTITION BY year ORDER BY year) AS elec_generated_world_percent,
	electricity_demand/SUM(electricity_demand) OVER (PARTITION BY year ORDER BY year) AS elec_demand_world_percent,
	net_electricity_supply/SUM(net_electricity_supply) OVER (PARTITION BY year ORDER BY year) AS net_elec_supply_world_percent
...
--------------------------------------------------------------------
--checking of table 1 world energy percentage query


SELECT SUM(hydro_elec_world_percent)
FROM
(SELECT *, 
	hydro_electricity/SUM(hydro_electricity) OVER (PARTITION BY year ORDER BY year) AS hydro_elec_world_percent,
	other_renewable_electricity/SUM(other_renewable_electricity) OVER (PARTITION BY year ORDER BY year) AS other_renewable_elec_world_percent,
